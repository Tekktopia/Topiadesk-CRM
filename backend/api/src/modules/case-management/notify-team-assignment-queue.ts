...
export async function enqueueTeamAssignmentNotification(data: NotifyTeamAssignmentJobData): Promise<void> {
  try {
    const q = getQueue();
    await q.add('notify-team-assignment', data satisfies NotifyTeamAssignmentJobData, { removeOnComplete: true, removeOnFail: 100 });
  } catch (err) {
