...
/**
 * Shared helper for every CRM BFF Route Handler under app/api/crm/**. Not a
 * `route.ts` file, so Next.js never registers it as an endpoint itself —
 * see lib/api/server-fetch.ts's header comment for why these proxies exist
 * at all (the API access token lives in an HttpOnly cookie invisible to
 * browser JS, so Client Components must go through a same-origin route
 * that attaches it server-side via `fetchApi()`).
 *
 * Forwards backend/api's response status + JSON body byte-for-byte
 * (including its error envelope shape from GlobalExceptionFilter —
 * `{ statusCode, message, path, timestamp }`) so client-side error handling
 * (see app/(crm)/_lib/api.ts's `apiFetch`) can read `.message` the same way
 * whether the failure came from validation, RLS, or a 404.
 */
export async function proxyJson(path: string, init?: RequestInit): Promise<NextResponse> {
  try {
    ...
    console.error(`[crm proxy] ${init?.method ?? 'GET'} ${path} failed`, err);
