...
/**
 * Upload dialog — POSTs multipart to app/api/documents (which forwards to
 * POST /documents), then optionally links the new document to a policy via
 * POST /api/documents/:id/links. Shared between the standalone /documents
 * manager and the policy detail's Documents tab (via `linkToPolicyId`),
 * per the build brief's guidance to keep this local to app/(policy)/
 * rather than adding a new shared packages/ui composite.
 */
export function UploadDocumentDialog({ linkToPolicyId, onUploaded }: { linkToPolicyId?: string; onUploaded: () => void }) {
    ...
    try {
      ...
      if (linkToPolicyId) {
        ...
        if (!linkRes.ok) {
          ...
          throw new Error(body?.message ?? 'Uploaded, but linking to the policy failed');
        }
      }
      ...
      <DialogContent>
        <form onSubmit={submit}>
          <DialogHeader>
            ...
            <DialogDescription>
              {linkToPolicyId ? 'Uploads a file and links it to this policy.' : 'Uploads a file to the document library.'}
            </DialogDescription>
          </DialogHeader>
