...
/** Mirrors app/(policy)/documents/upload-document-dialog.tsx's shape (that
 * file's header comment explains the upload -> link two-step), pointed at
 * `entityType: 'CARRIER'` instead of 'POLICY'. Local copy, same
 * cross-route-group reasoning as carrier-documents-panel.tsx. */
export function CarrierUploadDocumentDialog({ carrierId, onUploaded }: { carrierId: string; onUploaded: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!linkRes.ok) {
        ...
        throw new Error(body?.message ?? 'Uploaded, but linking to the carrier failed');
      }
      ...
      <DialogContent>
        <form onSubmit={submit}>
          <DialogHeader>
            ...
            <DialogDescription>Uploads a file and links it to this carrier.</DialogDescription>
          </DialogHeader>
