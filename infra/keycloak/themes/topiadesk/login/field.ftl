<#-- TopiaDesk override of keycloak.v2's own field.ftl — identical to the
     parent except `input`/`password` now carry a real `placeholder`
     attribute (the reference card's "Enter your username"/"Enter your
     password" ghost text). CSS alone can't add this: `::placeholder` only
     styles a placeholder that already exists on the element, it can't
     fabricate one, so a template override is the only way. Every other
     macro below (group/errorIcon/clipboard/checkbox) is untouched
     copy-paste from the parent, kept only so this file remains a complete,
     self-consistent field.ftl — if keycloak.v2 changes these upstream,
     this file needs re-syncing by hand. -->
  ...
  <@group name=name label=label error=error required=required>
    <span class="${properties.kcInputClass} <#if error?has_content>${properties.kcError}</#if>">
        <input id="${name}" name="${name}" value="${value}" type="text" autocomplete="${autocomplete}" <#if autofocus>autofocus</#if>
                placeholder="<#if name == 'username' || name == 'email'>Enter your username<#elseif name == 'password'>Enter your password</#if>"
